...
def test_authoritative_pod_uid_accepts_same_pod_fact_without_uid():
    authoritative = "k8s.pod:demo/api:uid-authoritative"
    name_only = "k8s.pod:demo/api"
    fact = _fact("same-pod-name-only", name_only)
    ledger = FactLedger.model_validate(
        _ledger("same-pod-name-only", name_only, [fact])
    )
    claim = _diagnosed_claim(
        [
            {
                "hypothesis_id": "hyp-name-only",
                "entity_id": authoritative,
                "summary": "Same Pod evidence without a UID",
                "supporting_fact_ids": [fact["fact_id"]],
                "contradicting_fact_ids": [],
                "unknowns": [],
                "confidence": 0.86,
...
def test_authoritative_pod_uid_rejects_different_explicit_uid():
    authoritative = "k8s.pod:demo/api:uid-authoritative"
    stale = "k8s.pod:demo/api:uid-stale"
    fact = _fact("same-name-stale-generation", stale)
    ledger = FactLedger.model_validate(
        _ledger("same-name-stale-generation", stale, [fact])
    )
    claim = _diagnosed_claim(
        [
            {
                "hypothesis_id": "hyp-stale-generation",
                "entity_id": authoritative,
                "summary": "A stale Pod generation must not cross the lane",
                "supporting_fact_ids": [fact["fact_id"]],
                "contradicting_fact_ids": [],
                "unknowns": [],
                "confidence": 0.86,
    ...
    assert fact["fact_id"] in result["claim_validation"]["invalid_fact_ids"]
