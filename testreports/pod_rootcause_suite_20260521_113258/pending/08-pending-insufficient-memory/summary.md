...
| pending-insufficient-memory | 8 | L1 ✅ | ✅ | Insufficient, memory, 100000Gi, 0/, nodes are available | - | - | 100% | ✅ | 132s |
...
| pending-insufficient-memory | 14 | L1 ✅ | ✅ | Insufficient, memory, 100000Gi, 0/, nodes are available | - | - | 75% | ✅ | 120s |
...
| pending-insufficient-memory | 23 | L1 ✅ | ✅ | Insufficient, memory, 100000Gi, 0/, nodes are available | - | - | 67% | ✅ | 94s |
| pending-insufficient-memory | 24 | L1 ✅ | ✅ | Insufficient, memory, 100000Gi, 0/, nodes are available | - | - | 67% | ✅ | 104s |
...
| pending-insufficient-memory | 29 | L1 ✅ | ✅ | Insufficient, memory, 100000Gi, 0/, nodes are available | - | - | 80% | ✅ | 127s |
...
| pending-insufficient-memory | 34 | L1 ✅ | ✅ | Insufficient, memory, 100000Gi, 0/, nodes are available | - | - | 100% | ✅ | 94s |
...
| pending-insufficient-memory | 44 | L1 ✅ | ✅ | Insufficient, memory, 100000Gi, 0/, nodes are available | - | - | 83% | ✅ | 99s |
...
| pending-insufficient-memory | 48 | L1 ✅ | ✅ | Insufficient, memory, 100000Gi, 0/, nodes are available | - | - | 100% | ✅ | 103s |
...
- 校正时间: 2026-05-26
- 校正标准: 最终报告明确指出 `rc-pending-insufficient-memory` 的 `resources.requests.memory=100000Gi` / `100TB` 远超任一节点内存容量，并导致 `0/3 nodes are available: 3 Insufficient memory`，判为根因正确。
- 误伤类型: `nodeSelector`/`PVC` 出现在“已排除/无此约束”的上下文中，不应计为冲突；`100TB`、`100,000Gi`、`requests.memory` 等表达按语义命中处理。
- 保留失败: #5、#13、#16、#19、#36、#41、#43、#45、#46、#47。主要原因是误判为集群内存耗尽、MemoryPressure、ResourceQuota/LimitRange、隐式默认 request 或存量 Pod requests 饱和。
- 人工校正后: 40/50 正确，根因准确率 80.0%。

## 场景汇总

| Case | Group | 根因准确率 | Runbook 覆盖率 | 平均证据率 | 平均 MTTR | 成功运行 |
|------|-------|------------|----------------|------------|-----------|----------|
| pending-insufficient-memory | pending | 80.0% | 100.0% | 85.6% | 1.2m | 50/50 |
