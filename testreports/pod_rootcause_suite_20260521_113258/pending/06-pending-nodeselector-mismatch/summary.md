...
| pending-nodeselector-mismatch | 4 | L1 ✅ | ✅ | didn't match, nonexistent-node-label | - | - | 100% | ✅ | 82s |
...
| pending-nodeselector-mismatch | 12 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 100% | ✅ | 90s |
...
| pending-nodeselector-mismatch | 15 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 100% | ✅ | 82s |
...
| pending-nodeselector-mismatch | 19 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 100% | ✅ | 91s |
...
| pending-nodeselector-mismatch | 28 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 100% | ✅ | 110s |
...
| pending-nodeselector-mismatch | 32 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 100% | ✅ | 101s |
...
| pending-nodeselector-mismatch | 35 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 100% | ✅ | 215s |
| pending-nodeselector-mismatch | 36 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 75% | ✅ | 98s |
| pending-nodeselector-mismatch | 37 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 100% | ✅ | 98s |
...
| pending-nodeselector-mismatch | 39 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 100% | ✅ | 96s |
...
| pending-nodeselector-mismatch | 47 | L1 ✅ | ✅ | nodeSelector, didn't match, nonexistent-node-label, node selector≈nodeselector | - | - | 100% | ✅ | 98s |
...
- 校正时间: 2026-05-26
- 校正标准: 最终报告明确指出 `spec.nodeSelector=aiops.e2e/nonexistent-node-label=true`，且集群所有节点均无该标签，导致 `0/3 nodes are available: node(s) didn't match Pod's node affinity/selector`，判为根因正确。
- 误伤类型: `pvc`、`Insufficient cpu` 出现在“已排除/非该问题”的上下文中，不应计为冲突；`node selector`、`Node-Selectors`、`spec.nodeSelector` 等表达按语义命中处理。
...
|------|-------|------------|----------------|------------|-----------|----------|
| pending-nodeselector-mismatch | pending | 100.0% | 100.0% | 95.0% | 1.1m | 50/50 |
